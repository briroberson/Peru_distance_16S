...
comparisons <- c("ins_latrine_out_latrine", "ins_latrine_ins_veg_patch", "out_latrine_ins_veg_patch") 
...
#read in sequences 
rep_seqs <- read_qza("PeruDec24_16s_rep-seqs.qza")$data
seq_df <- data.frame(
  ASV = names(rep_seqs),
  Sequence = as.character(rep_seqs),
  stringsAsFactors = FALSE)

taxaveg_top10_blast_16s <- simper_taxaveg_top10 %>%
  left_join(seq_df, by = "ASV")

write.csv(taxaveg_top10_blast_16s, "taxaveg_top10_blast_16s.csv") 
